...
TEST(DataLayoutTest, NullPointerValue) {
  // Default: AS0 null pointer is all-zeros. Unlisted AS are unknown unless
  // N<null-kind> is specified.
  {
    const DataLayout DL = cantFail(DataLayout::parse(""));
    std::optional<APInt> Val = DL.getNullPtrValue(0);
    ASSERT_TRUE(Val.has_value());
    EXPECT_TRUE(Val->isZero());
    EXPECT_EQ(Val->getBitWidth(), 64U);
    EXPECT_TRUE(DL.isNullPointerValueKnown(0));
    EXPECT_TRUE(DL.isNullPointerAllZeroes(0));

    std::optional<APInt> UnlistedVal = DL.getNullPtrValue(42);
    EXPECT_FALSE(UnlistedVal.has_value());
    EXPECT_FALSE(DL.isNullPointerValueKnown(42));
    EXPECT_FALSE(DL.isNullPointerAllZeroes(42));
  }

  // Explicit 'z' flag.
  {
    const DataLayout DL = cantFail(DataLayout::parse("pz1:32:32"));
    std::optional<APInt> Val = DL.getNullPtrValue(1);
    ASSERT_TRUE(Val.has_value());
    EXPECT_TRUE(Val->isZero());
  ...
  // Catch-all null model applies to address spaces without an override.
  {
    const DataLayout DL = cantFail(DataLayout::parse("Nz"));
    std::optional<APInt> Val = DL.getNullPtrValue(42);
    ASSERT_TRUE(Val.has_value());
    EXPECT_TRUE(Val->isZero());
    EXPECT_TRUE(DL.isNullPointerAllZeroes(42));
  }

  // AS-specific null override wins over the default null pointer value.
  {
    const DataLayout DL = cantFail(DataLayout::parse("Nz-po1:32:32"));
    std::optional<APInt> Val = DL.getNullPtrValue(1);
    ASSERT_TRUE(Val.has_value());
    EXPECT_TRUE(Val->isAllOnes());
    EXPECT_FALSE(DL.isNullPointerAllZeroes(1));

    std::optional<APInt> UnlistedVal = DL.getNullPtrValue(3);
    ASSERT_TRUE(UnlistedVal.has_value());
    EXPECT_TRUE(UnlistedVal->isZero());
    EXPECT_TRUE(DL.isNullPointerAllZeroes(3));
  }

  // Pointer specs without null flags inherit the default null pointer value.
  {
    const DataLayout DL = cantFail(DataLayout::parse("No-p1:32:32"));
    std::optional<APInt> Val = DL.getNullPtrValue(1);
    ASSERT_TRUE(Val.has_value());
    EXPECT_TRUE(Val->isAllOnes());
  }
  ...
  // Multiple address spaces with different null values.
  {
    ...
    // AS 1: all-ones
    std::optional<APInt> Val1 = DL.getNullPtrValue(1);
    ASSERT_TRUE(Val1.has_value());
    EXPECT_TRUE(Val1->isAllOnes());
    // AS 3: all-zeros
    std::optional<APInt> Val3 = DL.getNullPtrValue(3);
  ...
  // Default null pointer value parser.
  for (StringRef Str : {"Nz", "No"})
    EXPECT_THAT_EXPECTED(DataLayout::parse(Str), Succeeded());

  // Repeated default null pointer specifications are allowed; last one wins.
  {
    const DataLayout DL = cantFail(DataLayout::parse("Nz-No"));
    std::optional<APInt> Val = DL.getNullPtrValue(42);
    ASSERT_TRUE(Val.has_value());
    EXPECT_TRUE(Val->isAllOnes());
  }
  {
    const DataLayout DL = cantFail(DataLayout::parse("No-Nz"));
    std::optional<APInt> Val = DL.getNullPtrValue(42);
    ASSERT_TRUE(Val.has_value());
    EXPECT_TRUE(Val->isZero());
  }
  ...
  EXPECT_THAT_EXPECTED(
      DataLayout::parse("N"),
      FailedWithMessage(
          "malformed specification, must be of the form \"N<null-value>\""));
  EXPECT_THAT_EXPECTED(
      DataLayout::parse("Nn"),
      FailedWithMessage("unknown default null pointer value 'n'"));
  EXPECT_THAT_EXPECTED(
      DataLayout::parse("Nx"),
      FailedWithMessage("unknown default null pointer value 'x'"));
