...
class SPIRVNonSemanticDebugHandler : public DebugHandlerBase {
  ...
  // order, collected in beginModule() for DebugLexicalBlock emission. Both
  // node kinds map to the same NSDI instruction (SPIRV-LLVM-Translator's
  // transDbgScope treats them identically, modulo the Name operand).
  ...
  // DebugLexicalBlock result id per emitted DILexicalBlock/DINamespace (only
  // entries where emission succeeded); used to resolve Parent operands for
  // nested scopes and for entities scoped inside a namespace.
...
private:
  ...
  /// a \c DINamespace (SPIRV-LLVM-Translator's \c transDbgScope handles both
  /// with this single instruction). A \c DILexicalBlock supplies Line/Column
