...
// reference for the Parent operand. No-op when \p S is null or
// neither kind (e.g. the chain bottoms out at a DISubprogram or DIFile).
static void collectLexicalBlockChain(const DIScope *S,
                                     SetVector<const DIScope *> &Out) {
  if (!S || Out.contains(S) || !(isa<DILexicalBlock>(S) || isa<DINamespace>(S)))
    return;
  collectLexicalBlockChain(S->getScope(), Out);
  Out.insert(S);
}
