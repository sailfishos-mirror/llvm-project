...
bool LoadStoreVec::runOnRegion(Region &Rgn, const Analyses &RegionAnalyses) {
  ...
  bool IsStore = isa<StoreInst>(Bndl[0]);
  assert(all_of(Bndl,
                [IsStore](Instruction *I) {
                  return isa<StoreInst>(I) == IsStore;
                }) &&
         "Expected a homogeneous seed slice!");

  bool Changed = IsStore ? vectorizeStores(Bndl, Rgn)
