...
bool DevirtIndex::tryFindVirtualCallTargets(
  ...
  for (const TypeIdOffsetVtableInfo &P : TIdInfo) {
    ...
    for (auto VTP : VS->vTableFuncs()) {
      ...
      // If this is an alias to an internal-linkage symbol, we expect it to have
      // one summary (the alias). This would be an artifact of handing internal
      // linkage vtable entries during split module bitcode writing.
      if (TheFn.getSummaryList().size() == 1) {
        if (auto *AS =
                dyn_cast<AliasSummary>(TheFn.getSummaryList()[0].get())) {
          TheFn = AS->getAliaseeVI();
          assert(TheFn.hasLocal() &&
                 "The presence of an alias in the vtable is expected only as a "
                 "result of split module bitcode writing. The aliased symbol "
                 "should be internal linkage.");
        }
      }
