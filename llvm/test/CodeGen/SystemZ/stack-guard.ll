; RUN: opt -S -mtriple=s390x-linux-gnu -passes=stack-protector < %s | FileCheck -check-prefix=IR %s
; RUN: opt -S -mtriple=s390x-ibm-zos -passes=stack-protector < %s | FileCheck -check-prefix=IR %s
