...
; Exercise NonSemantic DebugLexicalBlock for a DINamespace used as the Parent
; of a DebugGlobalVariable: a nested (namespace-in-namespace) named case, and
; an anonymous C++ namespace nested inside a named one. Per the NSDI spec, an
; anonymous namespace's DebugLexicalBlock Name operand must be an empty
; OpString (clang emits `!DINamespace(scope: ...)` with no `name:` field for
; `namespace { ... }`, i.e. DINamespace::getName() == "").
