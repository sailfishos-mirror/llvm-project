// RUN: llvm-mc -triple=aarch64 -show-encoding -mattr=+sysinstr128,+d128,+tlbid,+tlb-rmi,+xs < %s \
// RUN:        | FileCheck %s --check-prefixes=CHECK-ENCODING,CHECK-INST
// RUN: not llvm-mc -triple=aarch64 -show-encoding -mattr=+tlb-rmi,+xs < %s 2>&1 \
// RUN:        | FileCheck %s --check-prefixes=CHECK-ERROR
// RUN: llvm-mc -triple=aarch64 -filetype=obj -mattr=+sysinstr128,+d128,+tlbid,+tlb-rmi,+xs < %s \
// RUN:        | llvm-objdump -d --mattr=+sysinstr128,+d128,+tlbid,+tlb-rmi,+xs --no-print-imm-hex - | FileCheck %s --check-prefix=CHECK-INST
// RUN: llvm-mc -triple=aarch64 -filetype=obj -mattr=+sysinstr128,+d128,+tlbid,+tlb-rmi,+xs < %s \
// RUN:        | llvm-objdump -d --mattr=-sysinstr128,-d128,+tlb-rmi,+xs --no-print-imm-hex - | FileCheck %s --check-prefix=CHECK-UNKNOWN
// Disassemble encoding and check the re-encoding (-show-encoding) matches.
// RUN: llvm-mc -triple=aarch64 -show-encoding -mattr=+sysinstr128,+d128,+tlbid,+tlb-rmi,+xs < %s \
// RUN:        | sed '/.text/d' | sed 's/.*encoding: //g' \
// RUN:        | llvm-mc -triple=aarch64 -mattr=+sysinstr128,+d128,+tlbid,+tlb-rmi,+xs -disassemble -show-encoding \
// RUN:        | FileCheck %s --check-prefixes=CHECK-ENCODING,CHECK-INST
...
// CHECK-ERROR: error: instruction requires: sysinstr128
