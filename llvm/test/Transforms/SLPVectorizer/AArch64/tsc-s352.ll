...
; CHECK-NEXT:    [[TMP5:%.*]] = insertelement <6 x float> poison, float [[TMP4]], i64 4
; CHECK-NEXT:    [[TMP6:%.*]] = insertelement <6 x float> [[TMP5]], float [[DOT_115]], i64 5
; CHECK-NEXT:    [[TMP7:%.*]] = shufflevector <4 x float> [[TMP0]], <4 x float> poison, <6 x i32> <i32 0, i32 1, i32 2, i32 3, i32 poison, i32 poison>
; CHECK-NEXT:    [[TMP8:%.*]] = shufflevector <6 x float> [[TMP6]], <6 x float> [[TMP7]], <6 x i32> <i32 6, i32 7, i32 8, i32 9, i32 4, i32 5>
; CHECK-NEXT:    [[TMP9:%.*]] = insertelement <6 x float> <float poison, float poison, float poison, float poison, float poison, float 1.000000e+00>, float [[TMP2]], i64 4
...
; CHECK-NEXT:    [[TMP13:%.*]] = extractelement <6 x float> [[TMP12]], i64 0
; CHECK-NEXT:    [[ADD23:%.*]] = fadd float [[DOT_115]], [[TMP13]]
; CHECK-NEXT:    [[TMP14:%.*]] = extractelement <6 x float> [[TMP12]], i64 1
; CHECK-NEXT:    [[ADD31:%.*]] = fadd float [[ADD23]], [[TMP14]]
; CHECK-NEXT:    [[TMP15:%.*]] = extractelement <6 x float> [[TMP12]], i64 2
; CHECK-NEXT:    [[ADD24:%.*]] = fadd float [[ADD31]], [[TMP15]]
; CHECK-NEXT:    [[TMP16:%.*]] = extractelement <6 x float> [[TMP12]], i64 3
; CHECK-NEXT:    [[ADD32:%.*]] = fadd float [[ADD24]], [[TMP16]]
; CHECK-NEXT:    [[TMP17:%.*]] = extractelement <6 x float> [[TMP12]], i64 4
; CHECK-NEXT:    [[ADD39]] = fadd float [[ADD32]], [[TMP17]]
