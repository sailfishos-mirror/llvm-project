...
; CHECK-EMPTY:
; CHECK-NEXT:    bb5:
; CHECK-NEXT:      BLEND ir<%phi5> = ir<%add2>/vp<[[VP4]]> ir<%add1>/ir<%c0> ir<%add3>/vp<[[VP15]]> ir<%add4>/vp<[[VP9]]> ir<%add4>/vp<[[VP9]]>
...
; CHECK-NEXT:      BLEND ir<%phi> = ir<1>/vp<[[VP9]]> ir<0>/vp<[[VP14]]> ir<0>/vp<[[VP14]]>
...
; CHECK-EMPTY:
; CHECK-NEXT:    bb3:
; CHECK-NEXT:      BLEND ir<%phi> = ir<0>/ir<%c0> ir<1>/vp<[[VP7]]> ir<1>/vp<[[VP7]]>
; CHECK-NEXT:      EMIT ir<%gep> = getelementptr ir<%p>, ir<%iv>
