...
void mlir::python::populateIRCore(nb::module_ &m) {
  ...
  nb::class_<PyLocation>(m, "Location")
      ...
      .def_prop_ro_static(
          ...
          // clang-format off
          nb::sig("def current(/) -> typing_extension.Optional[" MAKE_MLIR_PYTHON_QUALNAME("ir.Location") "]"),
          // clang-format on
  ...
  nb::class_<PyValue>(m, "Value")
      ...
      .def_prop_ro(
          ...
          // clang-format off
              nb::sig("def owner(self) -> typing_extension.Union[" MAKE_MLIR_PYTHON_QUALNAME("ir.Operation") ", " MAKE_MLIR_PYTHON_QUALNAME("ir.Block") ", NoneType]"))
      // clang-format on
      .def_prop_ro("uses",
                   [](PyValue &self) {
