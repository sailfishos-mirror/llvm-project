...
TEST_F(LlvmLibcRealpathTest, LongSymlinkGivesNameTooLong) {
  TestDir test_dir;
  ASSERT_TRUE(create_test_dir("SymlinkLoop", test_dir));
  ...
  // The link resolves to a the maximum length path,
  // so adding anything to the end means the intermediary path is too long.
  ASSERT_STREQ(realpath_buffered(test_dir.absolute_path("link/too_long")),
               nullptr);
  ASSERT_ERRNO_EQ(ENAMETOOLONG);
}
