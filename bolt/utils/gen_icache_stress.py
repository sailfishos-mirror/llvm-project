...
def generate_nop_block(nop_bytes, arch):
    """Generate an inline asm NOP block for the given arch and byte count."""
    if nop_bytes <= 0:
        return ''
    if arch == 'arm':
        num_nops = nop_bytes // 4
        nops = '\\n'.join(['nop'] * num_nops)
    else:  # x86
        num_nops = nop_bytes // 4
        nops = '\\n'.join(['.byte 0x0f, 0x1f, 0x40, 0x00'] * num_nops)
    return f'    __asm__ volatile("{nops}");\n'
...
def generate_compute_function_with_tiers(func_id, num_branches=15, nop_block=''):
    """Generate a function with 40/25/35 hot/warm/cold code ratio.

    Within each function:
    - 40% hot code (executed most of the time)
    - 25% warm code (rarely executed)
...
'''
    code += nop_block
    code += f'    uint64_t result = x ^ seed ^ {random.randint(1, 0xFFFFFFFF)}ULL;\n'
...
def generate_switch_function(func_id, num_cases=32, nop_block=''):
...
def generate_goto_function(func_id, num_labels=16, nop_block=''):
...
def main():
    ...
    parser.add_argument('--nop-bytes', type=int, default=0,
                        help='Insert N bytes of NOPs at the start of every function '
                             '(default: 0, must be a multiple of 4)')
    parser.add_argument('--arch', type=str, choices=['x86', 'arm'],
                        default='x86',
                        help='Target architecture for NOP encoding '
                             '(default: x86)')
    ...
    if args.nop_bytes < 0:
        parser.error('--nop-bytes must be non-negative')
    if args.nop_bytes % 4 != 0:
        parser.error('--nop-bytes must be a multiple of 4')

    random.seed(args.seed)

    nop_block = generate_nop_block(args.nop_bytes, args.arch)
    ...
    if args.nop_bytes > 0:
        print(f"  NOP bytes/func:    {args.nop_bytes} ({args.arch})")
    ...
    with open(args.output, 'w') as f:
        ...
        for func_type, func_id in all_functions:
            if func_type == 'hot':
                f.write(generate_compute_function_with_tiers(func_id, args.branches_per_func, nop_block))
            elif func_type == 'warm':
                f.write(generate_warm_function(func_id, args.branches_per_func // 2, nop_block))
            else:  # cold
                f.write(generate_cold_function(func_id, args.branches_per_func // 2, nop_block))
        ...
        if num_switch > 0:
            f.write("\n// ============ Switch Functions ============\n")
            for i in range(num_switch):
                f.write(generate_switch_function(i, nop_block=nop_block))

        # Computed goto functions
        if num_goto > 0:
            f.write("\n// ============ Computed Goto Functions ============\n")
            for i in range(num_goto):
                f.write(generate_goto_function(i, nop_block=nop_block))
