...
public:
  MetadirectiveBlockIVMarker(unsigned version, SymbolDSAGuard &dsaGuard)
      : version(version), dsaGuard(dsaGuard) {}
  ...
  bool Pre(const parser::DoConstruct &loop) {
    if (nestedOwnerDepth != 0 || !loop.IsDoNormal())
      return true;
...
markMetadirectiveBlockIVs(lower::pft::Evaluation &eval, unsigned version,
                          SymbolDSAGuard &dsaGuard) {
  const parser::Block *block = getMetadirectiveBlock(eval);
  assert(block && "block-associated metadirective must have a block");
  MetadirectiveBlockIVMarker marker(version, dsaGuard);
...
static void markMetadirectiveTaskImplicitSymbols(
  ...
  for (const semantics::Symbol *symbol : blockSymbols) {
    ...
    if (!semantics::omp::IsPrivatizable(*symbol) ||
      ...
      continue;
    dsaGuard.setSymbolDSA(const_cast<Symbol &>(*symbol),
...
static void genMetadirective(lower::AbstractConverter &converter,
  ...
  auto genVariant = [&](const parser::OmpDirectiveSpecification *spec) {
    ...
    if (consumesBody && llvm::any_of(queue, [ompVersion](const auto &item) {
      ...
      blockSymbols = markMetadirectiveBlockIVs(eval, ompVersion, dsaGuard);
