...
validateLocalABI(Ctx &ctx, const AMDGPUCallGraph &cg,
  ...
  for (auto &[sym, node] : cg) {
    ...
    if (localRes.waveSize == 32 && !targetInfo.supportsWave32()) {
      Err(ctx) << "AMDGPU: function '" << node->sym->getName()
    ...
    if (localRes.waveSize == 64 && !targetInfo.supportsWave64()) {
      Err(ctx) << "AMDGPU: function '" << node->sym->getName()
...
static AMDGPU::RawLinkingResources buildRawResources(const CGNode &kernel) {
  AMDGPU::RawLinkingResources info = kernel.info->propagatedRes;
...
patchKernelDescriptors(Ctx &ctx, AMDGPUCallGraph &cg,
  ...
  for (CGNode *kernel : cg.kernels()) {
    ...
    AMDGPU::ResolvedLinkingResources info =
        AMDGPU::resolveObjectLinkingResources(targetInfo,
                                              buildRawResources(*kernel));

    AMDGPU::patchKernelDescriptor(kdBuf, targetInfo, info, hasLDS, true);
...
static void patchHSAMetadata(Ctx &ctx, AMDGPUCallGraph &cg,
  ...
  for (InputSectionBase *sec : ctx.inputSections) {
    ...
    for (size_t i = 0, e = kernelsArray.size(); i < e; ++i) {
      ...
      AMDGPU::patchHSAMetadataKernel(kernMap, doc, info, hasLDS, true);
      modified = true;
    }
