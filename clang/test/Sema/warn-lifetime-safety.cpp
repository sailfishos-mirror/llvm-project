// RUN: %clang_cc1 -fsyntax-only -Wexperimental-lifetime-safety -verify %s
...
void potential_for_loop_use_before_free(MyObj safe) {
  MyObj* p = &safe;
  for (int i = 0; i < 1; ++i) {
    (void)*p;   // expected-note {{later used here}}
    MyObj s;
...
// No-Error Cases
//===----------------------------------------------------------------------===//
void no_error_dangle_then_rescue() {
  MyObj safe;
  MyObj* p;
  {
    MyObj temp;
