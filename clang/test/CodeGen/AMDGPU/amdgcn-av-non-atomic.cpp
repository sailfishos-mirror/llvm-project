...
void test_plain_store(int *p, int val) {
  [[clang::amdgcn_av("none")]] *p = val; // expected-warning {{'clang::amdgcn_av' attribute only applies to atomic expressions or Clang atomic builtins}}
}
...
void test_plain_call() {
  [[clang::amdgcn_av("none")]] foo(); // expected-warning {{'clang::amdgcn_av' attribute only applies to atomic expressions or Clang atomic builtins}}
}
...
void test_for_with_atom(int *p) {
  ...
  [[clang::amdgcn_av("none")]] for (;;) { // expected-warning {{'clang::amdgcn_av' attribute can only be applied to an expression}}
...
void test_if_atomic_condn(int *p) {
  [[clang::amdgcn_av("none")]] if (__atomic_load_n(p, __ATOMIC_ACQUIRE)) { // expected-warning {{'clang::amdgcn_av' attribute can only be applied to an expression}}
  }
}
