...
OriginList *OriginManager::getOrCreateList(const Expr *E) {
  ...
  if (auto *DRE = dyn_cast<DeclRefExpr>(E)) {
    ...
    if (doesDeclHaveStorage(DRE->getDecl())) {
      ...
      Root->setInnerOriginList(getOrCreateList(DRE->getDecl()));
    } else {
      // For reference-typed declarations (e.g., `int& r = p`) which no storage,
      // the DeclRefExpr directly reuses the declaration's list since references
      // don't add an extra level of indirection at the expression level.
      Root = getOrCreateList(DRE->getDecl());
    }
    return ExprToList[E] = Root;
  }
