...
public:
  enum class Kind : uint8_t {
    ...
    ReturnOfOrigin,
    // A pointer is used (eg. dereferenced).
    Use,
...
template <typename T>
static llvm::ImmutableSet<T>
getIntersection(const llvm::ImmutableSet<T> &A, const llvm::ImmutableSet<T> &B,
                typename llvm::ImmutableSet<T>::Factory &F) {
  llvm::ImmutableSet<T> Result = F.getEmptySet();
  for (const T &Elem : A)
    if (B.contains(Elem))
      Result = F.add(Result, Elem);
  return Result;
}
...
template <typename K, typename V, typename JoinerT>
static llvm::ImmutableMap<K, V>
join(llvm::ImmutableMap<K, V> A, llvm::ImmutableMap<K, V> B,
     typename llvm::ImmutableMap<K, V>::Factory &F, JoinerT Joiner) {
  if (A.getHeight() < B.getHeight())
    std::swap(A, B);
  ...
  for (const auto &Entry : B) {
    ...
    if (const V *ValA = A.lookup(Key))
      A = F.add(A, Key, Joiner(*ValA, ValB));
    else
      A = F.add(A, Key, ValB);
  }
  return A;
...
struct ExpiredLattice {
  ...
  void dump(llvm::raw_ostream &OS) const {
    ...
    for (const auto &Entry : Expired)
      OS << "  Loan " << Entry.first << " is expired\n";
  }
};
...
public:
  ExpiredLoansAnalysis(const CFG &C, AnalysisDeclContext &AC, FactManager &F,
                       LifetimeFactory &LFactory)
      : DataflowAnalysis(C, AC, F), Factory(LFactory.ExpiredLoanMapFactory) {}
...
// ========================================================================= //
//                       Checker and Error reporter
// ========================================================================= //

/// Struct to store the complete context for a potential lifetime violation.
struct PendingWarning {
  const Expr *IssueExpr;    // Where the loan was originally created.
  SourceLocation ExpiryLoc; // Where the variable went out of scope.
  const Expr *UseExpr;      // Where the pointer was dereferenced.
  Confidence Level;
};
...
public:
  ...
  void checkUse(const UseFact *UF) {
    ...
    // Identify loans that are both held by the pointer and have expired.
    ...
    for (LoanID L : HeldLoans) {
      if (AllExpiredLoans.contains(L))
        ...
        // If at least one loan is not expired, this is not a definite error.
    ...
    for (LoanID DefaultedLoan : DefaultedLoans) {
      ...
      assert(EF && "Could not find ExpireFact for a problematic loan.");
