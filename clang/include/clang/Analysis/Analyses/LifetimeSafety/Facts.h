...
class FactManager {
public:
  FactManager(AnalysisDeclContext &AC, const CFG &Cfg)
      : OriginMgr(AC.getASTContext(), AC.getDecl()), Cfg(Cfg), AC(AC) {
    BlockToFacts.resize(Cfg.getNumBlockIDs());
  }

  const CFGBlock *getBlock(const Fact *F) const;
  ...
  /// Calls C for all the predecessors of Fact F.
  void forAllPredecessors(const Fact *F,
...
private:
  ...
  llvm::DenseMap<const Fact *, const CFGBlock *> FactToBlockMap;
